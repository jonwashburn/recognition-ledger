...
Generate ONLY the proof code (no explanations):"""
    ...
    def find_sorries(self, file_path: Path) -> List[Dict]:
        ...
        # Find all lines with sorry that aren't comments
        for i, line in enumerate(lines):
            if 'sorry' in line and not line.strip().startswith('--') and not line.strip().startswith('#'):
                # Try to find the theorem/definition name
                theorem_name = "unknown"
                declaration = line.strip()
                
                # Look backwards for theorem/lemma/def declaration
                for j in range(i, max(0, i-20), -1):
                    prev_line = lines[j]
                    theorem_match = re.match(r'^\s*(theorem|lemma|def|example)\s+(\w+)', prev_line)
                    if theorem_match:
                        theorem_name = theorem_match.group(2)
                        # Get the full declaration up to :=
                        decl_lines = []
...
def main():
    ...
    # Priority files to solve - test with known sorry files
    priority_files = [
        Path("../formal/AxiomProofs.lean"),  # Has many sorries
        Path("../formal/MetaPrinciple.lean"),  # Has many sorries
        Path("../formal/Core/GoldenRatio.lean"),  # Has some sorries
        Path("../formal/Core/EightBeat.lean"),  # Has some sorries
    ]
