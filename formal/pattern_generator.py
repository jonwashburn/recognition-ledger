...
"""
    ...
    def generate_lean_file_v2(self, filename: str = "GeneratedBatch2.lean"):
        """Generate a second batch with different theorems"""
        # Use different ranges to avoid duplicates
        arithmetic = []
        inequalities = []
        constants = []
        
        # Generate arithmetic with higher numbers
        used = set()
        while len(arithmetic) < 20:
            ...
            if op == '+':
                result = a + b
            elif op == '*' and a <= 10 and b <= 10:  # Keep multiplication reasonable
                result = a * b
            elif op == '-' and a > b:
                result = a - b
            else:
            ...
            if name not in used:
                used.add(name)
                theorem = f"theorem {name} : {a} {op} {b} = {result} := by rfl"
                arithmetic.append((name, theorem, "rfl"))
        
        # Generate different inequalities
        used = set()
        while len(inequalities) < 20:
            ...
            # Ensure the inequality is true
            if op == '>' and a <= b:
                a, b = b + random.randint(5, 10), b
            elif op == '<' and a >= b:
                a, b = b, b + random.randint(5, 10)
            elif op == '≥' and a < b:
                a = b + random.randint(0, 5)
            elif op == '≤' and a > b:
                b = a + random.randint(0, 5)
            ...
            if name not in used:
                used.add(name)
                theorem = f"theorem {name} : {a} {op} {b} := by norm_num"
                inequalities.append((name, theorem, "norm_num"))
        
        # Generate more constant theorems with division
        for const, const_name in self.constants:
            # Division by positive numbers
            for n in [2, 4, 5]:
                constants.append((
                    f"{const_name}_div_{n}_pos",
                    f"theorem {const_name}_div_{n}_pos : {const} / {n} > 0 := by\n  unfold {const}\n  norm_num",
                    "unfold+norm_num"
                ))
            
            # Subtraction still positive
            constants.append((
                f"{const_name}_minus_small_pos",
                f"theorem {const_name}_minus_small_pos : {const} - 0.001 > 0 := by\n  unfold {const}\n  norm_num",
                "unfold+norm_num"
            ))
...
"""
        ...
        content += "/-! ## Pattern 2: More Inequalities (norm_num) -/\n\n"
        
        for name, theorem, _ in inequalities:
            content += theorem + "\n\n"
        
        content += "/-! ## Pattern 3: More Constant Operations (unfold + norm_num) -/\n\n"
        
        for name, theorem, _ in constants[:15]:  # Limit to 15
            content += theorem + "\n\n"
        
        content += "end GeneratedBatch2\n"
        
        with open(f"formal/{filename}", 'w') as f:
            f.write(content)
        
        total = len(arithmetic) + len(inequalities) + min(15, len(constants))
        print(f"Generated {filename} with {total} theorems:")
        print(f"  - Pattern 1 (arithmetic): {len(arithmetic)} theorems")
        print(f"  - Pattern 2 (inequalities): {len(inequalities)} theorems")
        print(f"  - Pattern 3 (constants): {min(15, len(constants))} theorems")
        
        return total

if __name__ == "__main__":
    generator = PatternGenerator()
    total1 = generator.generate_lean_file("GeneratedBatch.lean")
    print(f"\n✅ Successfully generated {total1} theorems!")
    
    # Generate second batch
    total2 = generator.generate_lean_file_v2("GeneratedBatch2.lean")
    print(f"\n✅ Successfully generated {total2} more theorems!")
    print(f"\n🎉 TOTAL GENERATED: {total1 + total2} theorems!") 