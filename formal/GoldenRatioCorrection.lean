...
theorem phi_unique_scaling :
  ...
  · -- Show uniqueness
    ...
    have hy_pos := hy.1
    have hy_eq := hy.2
    -- Both φ and y satisfy x^2 = x + 1, so x^2 - x - 1 = 0
    -- This quadratic has solutions (1 ± √5)/2
    -- Since y > 0, we must have y = (1 + √5)/2 = φ
    have h_quad : y^2 - y - 1 = 0 := by linarith [hy_eq]
    -- From quadratic formula and positivity constraint
    rw [φ]
    -- The positive root of x^2 - x - 1 = 0 is unique
    have h_discriminant : y = (1 + sqrt 5) / 2 := by
      -- y satisfies the quadratic and is positive
      have h_factor : (y - (1 + sqrt 5) / 2) * (y - (1 - sqrt 5) / 2) = 0 := by
        field_simp
        ring_nf
        exact h_quad
      cases' eq_or_eq_of_mul_eq_zero h_factor with h1 h2
      · exact h1
      · -- Rule out the negative root
        exfalso
        have h_neg : (1 - sqrt 5) / 2 < 0 := by norm_num
        rw [← h2] at h_neg
        linarith [hy_pos]
    exact h_discriminant -- Detailed quadratic analysis
           rw [h_quad] at h_val
           norm_num at h_val
