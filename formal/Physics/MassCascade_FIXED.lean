...
theorem mass_ratios_golden :
  ...
  by
  intros p1 p2 r1 r2 h1 h2
  cases h1 with _ _ m1
  cases h2 with _ _ m2
  simp [particle_mass_MeV]
  have mass_formula1 : particle_mass_MeV p1 = E_coh * φ^r1 := by
    exact mass_formula p1 r1 h1
  have mass_formula2 : particle_mass_MeV p2 = E_coh * φ^r2 := by
    exact mass_formula p2 r2 h2
  rw [mass_formula1, mass_formula2]
  field_simp
  have h_pos : E_coh > 0 := E_coh_positive
  have φ_pos : φ > 0 := φ_positive
  simp [h_pos, φ_pos]
  ring
  rw [pow_sub_comm φ_pos]
  rfl -- From cascade structure
