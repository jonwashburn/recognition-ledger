...
theorem hypercharge_quantization :
  ...
  by
  intro particle
  -- Hypercharge follows from SU(3) representation theory
  have h1 : ∃ (p q : Int), hypercharge_value particle = (p + 3*q)/6
  { apply hypercharge_su3_decomposition particle }
  
  rcases h1 with ⟨p, q, h_pq⟩
  -- Combine terms into single numerator
  have h2 : (p + 3*q)/6 = (p + 3*q)/6 := rfl
...
Note: This proof assumes the existence of a lemma `hypercharge_su3_decomposition` that establishes the SU(3) representation theory result for hypercharge quantization. The proof follows from the fact that hypercharge values arise from weights in SU(3) representations, which are always rational numbers with denominator 6 when expressed in standard normalization.

The key steps are:
1. Use SU(3) decomposition to express hypercharge as (p + 3q)/6
2. Combine terms into a single fraction
3. Define n as p + 3q to show the required form n/6 -- From residue structure
