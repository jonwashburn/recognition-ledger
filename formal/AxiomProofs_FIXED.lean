...
theorem axiom_completeness :
  ...
  have h_meta := h_derives h_new
  
  -- By the nature of MetaPrinciple, any derived axiom must be
  -- expressible in terms of the fundamental axioms
  
  -- We'll use cases to show it must reduce to existing axioms
  cases h_meta with
  | inl h1 => 
    left
    exact h1
