...
theorem critical_line_eight_beat_invariant :
  ...
  simp [CriticalLine, tick_transform]
  
  -- Split into real and imaginary parts
  have h_re : ∀ z ∈ CriticalLine, (tick_transform n z).re = (1/2 : ℝ) := by
    intro z hz
    simp [tick_transform]
    -- The tick transform preserves Re(s) = 1/2 by construction
    exact hz.1
    
  -- Show the transformed point remains on critical line
  constructor
  · -- Real part remains 1/2
    exact h_re s hs
  · -- Imaginary part changes but stays real
    simp [tick_transform]
    apply Complex.im_add_real
    · exact hs.2
    · -- Eight-beat shift preserves reality
      cases n
      all_goals { simp [EightBeat] } -- Geometric property of the transformation
