...
theorem recognitionEnergy_convergent (s : ℂ) (h : 1/2 < s.re) :
  ...
  apply Summable.of_norm_bounded_by_summable
intro p
split_ifs with h_irred
· -- Case: p is irreducible
  calc ‖‖debit s p - credit s p‖^2‖ 
    ≤ 4 * (gradeNat p : ℝ)^(-2 * s.re) := by
    apply h_bound p h_irred
· -- Case: p is not irreducible
  simp
  exact zero_le_all

-- Show the bounding series converges
have h_sum := Summable.mul_const 4 (fun p => (gradeNat p : ℝ)^(-2 * s.re))
apply h_sum.of_nonneg_of_le
· intro p
  exact pow_nonneg (nat_cast_nonneg _) _
· intro p
  split_ifs with h_irred
  · exact le_of_eq rfl
  · simp
...
· -- Use comparison test with p-series
  have h_two_re : 0 < 2 * s.re - 1 := by
    linarith [h]
  exact summable_nat_rpow_of_pos h_two_re -- This follows from the Prime Number Theorem
