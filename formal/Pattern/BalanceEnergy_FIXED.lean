...
theorem patternZeta_eq_zeta (s : ℂ) (h : 1 < s.re) :
  ...
  by
  simp [patternZeta]
  have h_conv : AbsolutelyConverges (fun n => 1 / (n : ℂ) ^ s) := by
    apply zeta_series_absolutely_convergent
    exact h
  
  have h_sum : ∑' (n : ℕ) (1 / (n : ℂ) ^ s) = zeta s := by
    exact zeta_eq_tsum h
    
  have h_bij : ∀ n : ℕ, ∃! p : Pattern, grade p = n := by
    intro n
    exact pattern_grade_bijective n
    
  have h_eq : ∑' (p : Pattern) (1 / (grade p : ℂ) ^ s) = ∑' (n : ℕ) (1 / (n : ℂ) ^ s) := by
    apply tsum_bij_of_injective grade
    · exact pattern_grade_injective
    · exact pattern_grade_surjective
    · intro p
      simp only [Pattern.grade_pos]
