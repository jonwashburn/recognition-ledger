...
lemma exists_irreducible_grade_between {a b : ℝ≥0} (h : a < b) :
  ∃ p : Pattern, IsIrreducible p ∧ a < grade p ∧ grade p < b := by
  -- Let's construct a pattern with grade between a and b
  let r := real.log (b/a) / real.log φ
  -- Take ceiling to ensure we get a valid pattern index
  let n := ⌈r⌉ 
  
  -- Construct the irreducible pattern at level n
  let p := Pattern.irreducible n
  
  exists p
  constructor
  · -- Prove p is irreducible
    exact Pattern.irreducible_is_irreducible n
  ...
  · -- Prove a < grade p
    calc
      a < a * φ^(r) := by
        ...
        exact pow_pos (golden_ratio_pos) _
      _ ≤ a * φ^n := by
        apply mul_le_mul_left
        exact pow_le_pow_of_le_left (golden_ratio_pos) (le_ceil _)
      _ = grade (Pattern.irreducible n) := by rfl
  
  · -- Prove grade p < b
    calc
      grade (Pattern.irreducible n) = a * φ^n := by rfl
      _ < a * φ^(r + 1) := by
        apply mul_lt_mul_left
        exact pow_lt_pow_of_lt_left (golden_ratio_pos) (ceil_lt_add_one _)
      _ = b := by
        rw [real.log_div_log] at r
