...
3. `rfl` confirms the resulting expressions are equivalent

The proof relies on the fact that NonTrivialZeros is defined as the intersection of CriticalStrip and the zeros of zeta, so not being in NonTrivialZeros means either not being in CriticalStrip or not being a zero of zeta.
