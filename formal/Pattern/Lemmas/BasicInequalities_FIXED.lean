...
lemma mem_critical_line {s : ℂ} :
  s ∈ {z : ℂ | z.re = 1/2} ↔ s.re = 1/2 := by
  constructor
  · intro h
    exact SetMem.mem_def.mp h
  · intro h
    exact SetMem.mem_def.mpr h

This proof breaks down into:
1. Use `constructor` to split into both directions of ↔
2. Forward direction: `intro h` brings hypothesis into context, `exact SetMem.mem_def.mp h` proves it
3. Reverse direction: `intro h` for new hypothesis, `exact SetMem.mem_def.mpr h` proves it

The proof uses the set membership definition directly since the set is defined using a predicate that exactly matches the right-hand side of the equivalence.
