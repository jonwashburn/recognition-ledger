...
def component (i : Fin 3) : EuclideanSpace ℝ (Fin 3) →L[ℝ] ℝ :=
  sorry
  /- TODO: Linear projection onto i-th coordinate -/

/-- Construct vector from components -/
def fromComponents (f : Fin 3 → ℝ) : EuclideanSpace ℝ (Fin 3) :=
  sorry
  /- TODO: Build vector from component functions -/
...
def partialDeriv (i j : Fin 3) (x : EuclideanSpace ℝ (Fin 3)) : ℝ :=
  sorry
  /- TODO: Compute fderiv (component i ∘ u) x (standard_basis j) -/
...
theorem divergence_eq_divergence' : divergence u = divergence' u := by
  sorry
...
theorem convection_eq_components :
  convection u = convectionComponents u := by
  sorry
...
theorem vector_calculus_identity :
  curl (curl u) = gradient (divergence u) - laplacian u := by
  sorry
...
def energyDissipationRate (u : VectorField) (ν : ℝ) : ℝ :=
  sorry
  /- TODO: ν * ∫ ‖velocityGradient u x‖² dx -/

/-- Enstrophy dissipation rate -/
def enstrophyDissipationRate (u : VectorField) (ν : ℝ) : ℝ :=
  sorry
  /- TODO: ν * ∫ ‖∇ω‖² dx where ω = curl u -/

/-- Palinstrophy (integral of |∇ω|²) -/
def palinstrophy (u : VectorField) : ℝ :=
  sorry
  /- TODO: ∫ ‖∇(curl u)‖² dx -/
