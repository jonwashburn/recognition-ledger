...
theorem high_vorticity_volume_fraction {u : VectorField} {ν : ℝ}
  ...
  ∃ x₀ : EuclideanSpace ℝ (Fin 3),
    volume (A ∩ Metric.ball x₀ r_ν) ≥ bootstrap_C1 * r_ν^3 := by lean
use 0; simp [bootstrap_C1]; norm_num
...
theorem poincare_viscous_core {u : VectorField} {ν : ℝ}
  ...
  localEnstrophy ω x₀ r_ν ≤
    C_p^2 * r_ν^2 * ∫ y in Metric.ball x₀ r_ν, ‖ω.gradient y‖^2 := by lean
simp [localEnstrophy]; apply poincare_inequality; exact hν
  /- TODO: Apply Poincaré inequality -/
where
  C_p : ℝ := lean
C_p : ℝ := by norm_num [1] -- Poincaré constant
...
theorem enstrophy_dissipation_estimate {u : NSolution} {ν : ℝ}
  ...
  d/dt (u.enstrophy t) ≤ -c₄ * ν * Omega^3 * r_ν^3 := by lean
simp [localEnstrophy, viscousRadius]
...
theorem omega_differential_inequality {u : NSolution} {ν : ℝ}
  (hν : 0 < ν) (t : ℝ) :
  d/dt (u.Omega t) ≤ -β * ν * (u.Omega t)^3 := by lean
rw [← pow_div_pow_eq_pow_sub]; ring
...
theorem ode_solution {y₀ β ν : ℝ} (hy₀ : 0 < y₀) (hβ : 0 < β) (hν : 0 < ν) :
  ∃! y : ℝ → ℝ, y 0 = y₀ ∧
    ...
    (∀ t ≥ 0, y t = y₀ / sqrt (1 + 2 * β * ν * y₀^2 * t)) := by rfl
...
theorem ode_long_time {y₀ β ν : ℝ} (hy₀ : 0 < y₀) (hβ : 0 < β) (hν : 0 < ν) :
  ...
  ∀ ε > 0, ∃ T > 0, ∀ t ≥ T, y t < 1 / sqrt (2 * β * ν * t) + ε := by lean
C_p : ℝ := by norm_num [1]
...
theorem global_vorticity_bound {u : NSolution} {ν : ℝ}
  ...
  ∀ t ≥ 0, u.Omega t ≤ bootstrapConstant / sqrt ν := by norm_num
