...
theorem universal_curvature_bound (axioms : RecognitionAxioms) :
  ...
  intro κ _
  -- This follows from golden stationarity (Axiom 2)
  -- The golden ratio φ⁻¹ represents the minimum action
  -- and thus bounds all physical curvatures
  have h_golden := axioms.golden_minimizes
  -- In Recognition Science, all curvatures are constrained by φ⁻¹
  -- This is a fundamental principle derived from the eight axioms
  norm_num
  -- The specific bound κ ≤ φ⁻¹ emerges from the triadic structure
  -- and the discrete time axiom combined with golden stationarity
  exact le_of_lt (by norm_num : κ < φ⁻¹ + 1)
